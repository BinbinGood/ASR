...
class FeatureNormalizer(object):
    ...
    :type num_samples: int,传入-1表示所有音频
    ...
    def _compute_mean_std(self, manifest_path, num_samples, num_workers):
        """从随机抽样的实例中计算均值和标准差"""
        manifest = read_manifest(manifest_path)  # 读取数据，并以字典格式保存
        if num_samples < 0 or num_samples > len(manifest):
            sampled_manifest = manifest
        else:
            sampled_manifest = self._rng.sample(manifest, num_samples)  # 采样只保存要求的样本个数
...
class NormalizerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # sampled_manifest的key: "audio_filepath", "duration", "text"
        instance = self.sampled_manifest[idx]
        # 利用频域分析，获取音频特征
        audio = AudioSegment.from_file(instance["audio_filepath"])  # 根据音频路径读取音频并进行处理
